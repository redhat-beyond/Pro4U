...
class TestSchedule:
    def test_create_schedule(self, client, professional, save_schedule):
        response = client.get(reverse('schedule_new', kwargs={'professional': professional.pk}))
        ...
        response = client.post(reverse('schedule_new', kwargs={'professional': professional.pk}), data)
        assert response.status_code == 302
        assert Schedule.objects.filter(professional_id=professional).count() == 2
        assert response.url == reverse('calendar', kwargs={'professional': professional.pk})
    ...
    def test_create_schedule_invalid_data(self, client, professional, save_schedule):
        ...
        assert response.url == reverse('schedule_new', kwargs={'professional': professional.pk})
