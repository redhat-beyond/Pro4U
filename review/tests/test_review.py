...
class TestReview:
    def test_get_new_review(self, client, professional, create_review: Callable[[str, str, str, str, int], Review]):
        review = create_review(client, professional, '4', DESCRIPTION, DAYS)  # Using factory function
        assert review in Review.objects.all()

    def test_get_deleted_review(self, client, professional, create_review: Callable[[str, str, str, str, int], Review]):
        review = create_review(client, professional, '4', DESCRIPTION, DAYS)  # Using factory function
        Review.objects.filter(id=review.id).delete()
        assert review not in Review.objects.all()

    def test_method_filter_by_professional(self, client, professional,
    ...
    def test_method_get_professional_avg_rating(self, client, client2, professional,
                                                create_review: Callable[[str, str, str, str, int], Review]):
    ...
    def test_method_str(self, client, professional, create_review: Callable[[str, str, str, str, int], Review]):
        review = str(
            create_review(client, professional, '4', DESCRIPTION, DAYS)
